...
#
    ...
    def scroll(explorer):
        ...
        while vimpel is False and explorer is not None: # När en viss explorer saknas
            ...
            hittad_explorer = ProgressDialog.hitta_scroll(f"./src/img/expl_{individ}.bmp", faktor)
            safestop += 1
            if hittad_explorer:
                vimpel = True
                return vimpel
    ...
    def berakna_command(bild_sokvag, faktor): # Hitta sökområdet för typer och check. # Behöver finslipas
        global command_area
        bild = Image.open(bild_sokvag)
        skalad_bild = bild.resize((int(bild.width * faktor), int(bild.height * faktor)))
        bild_array = np.array(skalad_bild)  # Konvertera PIL-bilden till en array
    ...
    def hitta_explorer(self, bild_sokvag, faktor):
        ...
        for _ in range(3): # Loopa 3ggr om den INTE hittar
            ...
            if hittad_position is not None:
                ...
                time.sleep(0.1)
                ProgressDialog.hitta_typ(f"./src/img/typ_{typ}.bmp", faktor)
                ProgressDialog.hitta_tid(f"./src/img/tid_{sort}_{tid}.bmp", faktor)
                ProgressDialog.popup_flagga.set()
        ...
        for explorer in explorers:
            ...
            while flagga:
                ...
                if hittad_explorer is not None:
                    ...
                    hitta = ProgressDialog.scroll(explorer)
