...
#
### Finns en ide här om att pröva explorer vs explorer, för att köra båda i samma fil
...
def hitta_scroll(bild_sokvag, faktor):
    ...
    if hittad_position is None:
        pyautogui.scroll(-2) # Scrollar neråt
        time.sleep(0.01)
        # Och här loopas hitta_scroll genom funktionen scroll.

def scroll(): # Letar efter alla explorers en efter en.
    global explorers
    global explorer
    for i, explorer in enumerate(explorers):
        explorer_index = i
        hitta_scroll(f"img/expl_{explorers[explorer_index]}.bmp", faktor)
